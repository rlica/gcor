...
int initialize()
{
  ...
  else
  {
    ...
    {
      printf("ERROR: Cannot create 'gcor_settings.txt'\n");
      exit(0);
    }
    fprintf(settings,
            "Channels=8192\n\
...
void centerMax(double *refSpec)
{ // will center each region at the highest channel inside it
  ...
  for (i = 0; i < regions; i++)
  {
    ...
    if (chan[i] - width[i] / 2 < low + shift)
      chan[i] += (low + shift) - (chan[i] - width[i] / 2); // to avoid going beyond the low limit
    if (chan[i] + width[i] / 2 > high - shift)
      chan[i] -= (width[i] / 2 + chan[i]) - (high - shift); // to avoid going above the high limit
  }
  for (i = 1; i < regions; i++)
  {
    if (chan[i] - chan[i - 1] < width[i] / 2)
      width[i] = 0; // to remove overlapping regions
  }
}
...
int readData(int **rawSpec, int *irun, int *irunref)
{
  ...
  for (i = 1; i <= runstop; i++)
  {
    ...
    if (!fopen(infile, "rb"))
    {
      if (i == runref)
      {
        printf("ERROR: Cannot open reference %2s.%04d\n", name, runref);
        exit(0);
      }
      else
        continue;
  ...
  if (runCount < 2)
  {
    printf("ERROR: Could not read more than the reference run\n");
    exit(0);
  }
  else
    printf("----> %d runs were successfully read\n", runCount);
...
double chisq(double *spec1, double *spec2, int l1, int l2, int n)
{ // chisq of two regions
...
double ratio(double *spec, int l, int n)
{ // checks if there is a significant change in the region;
  int i;
...
void shift(double *smoothSpec, double *spec1, double *spec2, struct Data2Fit *shData)
{ // finds shift and error for each region by sweeping and minimizing chiSq
...
int performFit(struct Data2Fit *shData, double *chisq, double *coeff)
{
  ...
  if (j < 2)
    n = 0; // not enough points to perform fit
  else
    polynomialfit(n, degree, xi, yi, ei, chisq, coeff);
...
void gnuplot(FILE *gnuplotPipe, int irun, int idet, struct Data2Fit *shData, int n, double chisq, double *coeff, double norm)
{
  ...
  if (degree == 2)
    sprintf(plot, "plot [0:3000][-%d:%d]'gcor_fit-data.temp' using 1:2:3 with yerrorbars, %lf + %lf*x", sweep, sweep, coeff[0], coeff[1]);
  else if (degree == 3)
    sprintf(plot, "plot 'gcor_fit-data.temp' using 1:2:3 with yerrorbars, %lf + %lf*x + %lf*x*x", coeff[0], coeff[1], coeff[2]);
  else if (degree == 4)
    sprintf(plot, "plot 'gcor_fit-data.temp' using 1:2:3 with yerrorbars, %lf + %lf*x + %lf*x*x %lf*x*x*x", coeff[0], coeff[1], coeff[2], coeff[3]);
...
void writeCal(FILE *fo, double *coeff, int irun, int idet)
{
  ...
  {
    if (deg == 0)
      fprintf(fo, "%9.3f", coeff[deg]);
    else if (deg == 1)
      fprintf(fo, "%10.6f", coeff[deg]);
    else
      fprintf(fo, "%15.6E", coeff[deg]);
  }
  fprintf(fo, "\n");
  fflush(fo);
}
...
double calibrate(double *refSpec, double *Spec, double *coeff)
{
  ...
  {
    if (calSpec[j] < 0.5 * calSpec[j - 1])
      calSpec[j] = (calSpec[j - 1] + calSpec[j + 1]) / 2;
    if (calSpec[j] > 1.5 * calSpec[j - 1])
      calSpec[j] = (calSpec[j - 1] + calSpec[j + 1]) / 2;
    Spec[j] = calSpec[j];
  }
