...
void readMaxDeg(FILE *fi, int *maxdeg) // v1.3
{
  ...
  {
    if (maxdeg[col] < deg[line][col])
      maxdeg[col] = deg[line][col];

    for (k = 0; k < deg[line][col]; k++)
      fscanf(fi, "%*f");
    fscanf(fi, "%*d"); // limit1);
...
int main(int argc, char **argv)
{
  ...
  while (fscanf(fi, "%d %d %d %d", &runnum, &detnum, &regions, &deg1) == 4)
  {
    ...
    for (k = 0; k < deg1; k++)
    {
      fscanf(fi, "%lf", &pol1[k]);
      if (k == 0)
        fprintf(fo, "%9.3f", pol1[k]);
      else if (k == 1)
        fprintf(fo, "%10.6f", pol1[k]);
      else
        fprintf(fo, "%15.6E", pol1[k]);
      if (k == deg1 - 1)
      {
        while (k + 1 < maxdeg[reg])
        {
          fprintf(fo, "               ");
          k++;
        } // v1.3
    ...
    for (i = 0; i < regions - 1; i++)
    {
      ...
      { // Secant method
        ...
        if (control > 10000000)
        {
          printf("Warning! Detector #%02d Regions %d-%d: Functions do not intersect.\n", detnum, i, i + 1);
          break;
        }
      } while (t > EPS);
      ...
      for (k = 0; k < deg2; k++)
      {
        pol1[k] = pol2[k];
        if (k == 0)
          fprintf(fo, "%9.3f", pol2[k]);
        else if (k == 1)
          fprintf(fo, "%10.6f", pol2[k]);
        else
          fprintf(fo, "%15.6E", pol2[k]);
        if (k == deg2 - 1)
        {
          while (k + 1 < maxdeg[reg])
          {
