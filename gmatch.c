...
double ChanFromEnergy(double *coeff, int degree, double energy)
{
  ...
  { // Secant method
    ...
    if (control > 10000000)
    {
      printf("ERROR! Cannot find polynomial roots.\n");
      break;
    }
  } while (t > EPS);
...
int readMCal(FILE *fi, double *coeff, int idet, double energy)
{
  ...
  while (fscanf(fi, "%*d %d %d %d", &index, &regions, &degree) != EOF)
  {
    ...
    {
      for (k = 0; k < degree; k++)
        fscanf(fi, "%lf", &coeff[k]);
      fscanf(fi, "%d", &limit1);
      chan = ChanFromEnergy(coeff, degree, energy);
      if (chan < limit1 && chan > 0)
        return degree;
      else
      ...
      fgets(string, sizeof string, fi); // reads a line from the file
  }
...
int main(int argc, char **argv)
{
  ...
  while (fscanf(listFile, "%d %lf %lf", &index, &energy, &list_channel) != EOF)
  {
    ...
    if (strcmp(ext, ext_mcal) == 0)
    {
      degree = readMCal(calFile, coeff, index, energy);
    }
    if (strcmp(ext, ext_cal) == 0)
    {
      degree = readCal(calFile, coeff, index, energy);
    }
